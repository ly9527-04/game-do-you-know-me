...
export function Leaderboard({ creatorNickname, challengeCount, averageScore, entries, shareUrl }: LeaderboardProps) {
  ...
  return (
    <section className="leaderboard" aria-labelledby="leaderboard-title">
      ...
      {entries.length ? <ol className="leaderboard__entries">{entries.map((entry, index) => <li key={entry.attemptId}><span>{index + 1}</span><strong>{entry.nickname}</strong><b>{entry.score} 分</b><time dateTime={entry.createdAt}>{formatDate(entry.createdAt)}</time></li>)}</ol> : <p>还没有朋友来挑战，分享链接试试看。</p>}
